...
以下参数是 EMNIST 正式实验的起始设置。当前论文主表采用单随机种子协议以节省计算资源，统一使用 `seed=42`。若后续时间允许，可将关键结论补充为多随机种子稳健性分析，但本手册中的主表命令不再默认运行 `42/43/44` 三组随机种子。
...
汇总结果前，应保留每一个原始运行目录。只在单独的分析目录中汇总最终指标 CSV，绝不能覆盖原始审计、监管或污染日志。当前主表采用单 seed42，因此汇总表中的 `std` 仅用于脚本格式兼容，不应被解释为多随机种子稳定性。

## 8. seed42 汇总与论文主表

每个正式命令使用 `seed=42` 单独运行，并保持稳定的命名前缀。新版 DP-CNN/GRAIL-FL 主基线实验建议使用 `grail_main_dpcnn_emnist_seed42` 等名称；程序会自动追加时间戳。准备论文最终主表时不要再使用旧 APDP 结果前缀。
...
| 输出文件 | 用途 |
| --- | --- |
| `experiment_seed_metrics.csv` | 每个原始运行目录中的每种方法一行，并从目录名称推断 seed。当前主表应只有 seed42。 |
| `experiment_metric_summary.csv` | 长表形式的 `method-metric-mean-std-n` 汇总；单 seed 下 `std` 不作为稳健性证据。 |
| `experiment_paper_main_table.csv` | 宽表形式，包含 `<metric>_mean`、`<metric>_std` 和 `<metric>_n` 列；单 seed 下主要读取 mean/n。 |
